...
def train_one_epoch(model: MeMOTR, train_states: dict, max_norm: float,
    ...
    for i, batch in enumerate(dataloader):
        ...
        # long enough for the gated short-/long-term path to reach the loss)
        # yields a loss with no grad_fn, and loss.backward() would raise.
        # Upstream never hits this because the backbone always requires grad.
        # Skip backward for such clips (they contribute zero gradient anyway);
        # inert for normal runs where the loss always requires grad.
