#!/usr/bin/env python3
"""B4 — combined long-term (signal) + short-term (symmetric) gates: train +
per-checkpoint full-val driver.

For each seed it:
  1. writes a per-seed config (configs/B4.yaml with SEED + OUTPUTS_DIR patched)
     into  outputs/b4/seed_<S>/config_seed<S>.yaml,
  ...
  3. runs full-val on ALL checkpoints  (python main.py --mode eval, EVAL_MODE=continue).
     submit() rebuilds the combined-gate model from <seed_dir>/train/config.yaml,
     so no extra wiring is needed; TrackEval writes pedestrian_summary.txt per ckpt.

B4 initializes from the released checkpoint (PRETRAINED_MODEL in B4.yaml,
strict=False) — NOT warm-started from B3c/B2 checkpoints, by design (see the
header of configs/B4.yaml). Both gates train from their fresh init.

Pick the best epoch per seed with:
    python gating/analysis/pick_best.py outputs/b4/seed_42 --split val
...
Usage (from repo root):
    python gating/scripts/run_b4.py                        # seed 42
    python gating/scripts/run_b4.py --seeds 42 123
    python gating/scripts/run_b4.py --skip-grad-check
    python gating/scripts/run_b4.py --dry-run
"""
import argparse
import os
import subprocess
...
def main():
    ...
    ap.add_argument("--base-out", default=os.path.join(REPO_ROOT, "outputs", "b4"))
    ...
    if not args.skip_grad_check:
        print("[B4] running mandatory gradient check before training ...")
        run([sys.executable, os.path.join("gating", "scripts", "grad_check_b4.py"),
    ...
    print("Done. Pick the best epoch with:")
    for seed in args.seeds:
