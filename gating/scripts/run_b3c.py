#!/usr/bin/env python3
"""B3c — signal-driven lambda: train + per-checkpoint full-val driver.

For each seed it:
  1. writes a per-seed config (configs/B3c.yaml with SEED + OUTPUTS_DIR patched)
     into  outputs/b3c/seed_<S>/config_seed<S>.yaml,
  ...
  3. runs full-val on ALL checkpoints  (python main.py --mode eval, EVAL_MODE=continue).
     submit() rebuilds the (signal-gate) model from <seed_dir>/train/config.yaml,
     so no extra wiring is needed; TrackEval writes pedestrian_summary.txt per ckpt.

Pick the best epoch and plot the signal/lambda analysis with:
...
Per IMPLEMENTATION_PLAN B3c, also report the signal distributions to the human
...
Usage (from repo root):
    python gating/scripts/run_b3c.py                       # seeds 42 123
    python gating/scripts/run_b3c.py --seeds 42
    python gating/scripts/run_b3c.py --skip-grad-check
    python gating/scripts/run_b3c.py --dry-run
"""
import argparse
import os
import subprocess
...
def main():
    ...
    ap.add_argument("--base-out", default=os.path.join(REPO_ROOT, "outputs", "b3c"))
    ...
    if not args.skip_grad_check:
        print("[B3c] running mandatory gradient check before training ...")
        run([sys.executable, os.path.join("gating", "scripts", "grad_check_b3c.py"),
    ...
    print(f"  python gating/analysis/plot_b3c.py {args.base_out}")
    for seed in args.seeds:
