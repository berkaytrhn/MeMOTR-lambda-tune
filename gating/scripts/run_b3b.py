#!/usr/bin/env python3
"""B3b — per-channel learnable lambda vector: train + per-checkpoint full-val driver.

For each seed it:
  1. writes a per-seed config (configs/B3b.yaml with SEED + OUTPUTS_DIR patched)
     into  outputs/b3b/seed_<S>/config_seed<S>.yaml,
  ...
  3. runs full-val on ALL checkpoints  (python main.py --mode eval, EVAL_MODE=continue).
     submit() rebuilds the (vector-gate) model from <seed_dir>/train/config.yaml,
     so no extra wiring is needed; TrackEval writes pedestrian_summary.txt per ckpt.

B3b trains for 3 epochs over 2 seeds (42, 123) per the user's instruction, with
the released DanceTrack-paper training hyperparameters kept (only the gating
mechanism, epoch budget and seed count change vs. the paper).

Pick the best epoch and plot the per-channel lambda distribution with:
...
Usage (from repo root):
    python gating/scripts/run_b3b.py                       # seeds 42 123
    python gating/scripts/run_b3b.py --seeds 42
    python gating/scripts/run_b3b.py --skip-grad-check
    python gating/scripts/run_b3b.py --dry-run
"""
import argparse
import os
import subprocess
...
def main():
    ...
    ap.add_argument("--base-out", default=os.path.join(REPO_ROOT, "outputs", "b3b"))
    ...
    if not args.skip_grad_check:
        print("[B3b] running mandatory gradient check before training ...")
        run([sys.executable, os.path.join("gating", "scripts", "grad_check_b3b.py"),
    ...
    print(f"  python gating/analysis/plot_b3b.py {args.base_out}")
    for seed in args.seeds:
